...
class BaseOptions():
	...
	def initialize(self):
		...
		self.parser.add_argument('--env', type=str, default='net34', help='# use gpu train')
...
def train():
	...
	kernel_blur = loadmat('./kernel_blur.mat')
	...
	for epoch in range(1,opt.max_epoch+1):
		...
		for ii,(img, labels)  in enumerate(dataloader):
			...
			for l in range(0,opt.batch_size):
				...
				kernelBlur = kernel_blur[int(kernel_count)]				
		...
		if epoch%1 == 0:
			...
			for ii,(img, labels)  in enumerate(dataloader_validata):
				...
				for l in range(0, 100):
					...
					validata_kernelBlur = kernel_blur[int(validata_kernel_count)]
